...
def parallel_generations(
    ...
    use_fiddler = getattr(args, "use_fiddler", False)
    simulation_model = getattr(args, "simulation_model", None)

    if use_fiddler or simulation_model is not None:
        # Fiddler model: skip accelerate, model is already on CUDA
        pass
    elif args.max_memory_per_gpu is not None:
